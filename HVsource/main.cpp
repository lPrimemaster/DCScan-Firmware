...
void updateLCDRealValues(int v1, int v2, float i1, float i2)
{
    ...
    lcd.printf("page0.i1r.val=%.2f", i1); final();
    lcd.printf("page0.i2r.val=%.2f", i2); final();
}
...
void updateLCDTargetValues(int v1, int v2, float i1, float i2)
{
    ...
    if(i1 > -1) { lcd.printf("page0.i1r.val=%.2f", i1); final(); }
    if(i2 > -1) { lcd.printf("page0.i2r.val=%.2f", i2); final(); }
}
...
int main()
{
    wait(2); // VER COMO ISTO FUNCIONA SO COM O LCD (?)
    cs = 1;
    en1 = 0;
    en2 = 0;
    spi.format(8, 0);
